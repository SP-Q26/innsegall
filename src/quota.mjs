...
export function formatQuotaBlocked(q, d = new Date(), reason = "limit_reached") {
  ...
  return [
    ...
    "",
    "Need a panic scout · press 1 at the mist gate, or:",
    `  innsegall scout   (opens Stripe Checkout · $${PRICING.extra_run.usd.toFixed(2)})`,
    `  innsegall supplies extra   (alias: innsegall buy extra)`,
    `  After pay · innsegall import`,
    `  Then · innsegall run`,
    `  Protect files · innsegall backup  (Time Machine + cloud)`,
    "",
    "Or wait for the next free Voyage · or Clan:",
    `  innsegall supplies clan   · innsegall open supplies`,
